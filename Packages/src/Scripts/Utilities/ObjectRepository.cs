...
namespace Coffee.UISoftMask
{
    internal class ObjectRepository<T> where T : Object
    {
        ...
        {
            // Find existing entry.
            Profiler.BeginSample("(SM4UI)[ObjectRepository] Valid > Find existing entry");
            for (var i = 0; i < _cache.Count; ++i)
            {
                var entry = _cache[i];
        ...
        {
            // Find existing entry.
            Profiler.BeginSample("(SM4UI)[ObjectRepository] Get > Find existing entry");
            for (var i = 0; i < _cache.Count; ++i)
            {
                var entry = _cache[i];
                if (entry.hash != hash) continue;
            ...
            // Create new entry.
            Profiler.BeginSample("(SM4UI)[ObjectRepository] Get > Create new entry");
        ...
        public void Release(ref T obj)
        {
            ...
            Profiler.BeginSample("(SM4UI)[ObjectRepository] Release");
            for (var i = 0; i < _cache.Count; i++)
            {
                var entry = _cache[i];
                ...
                {
                    Profiler.BeginSample("(SM4UI)[ObjectRepository] Release > RemoveAt");
