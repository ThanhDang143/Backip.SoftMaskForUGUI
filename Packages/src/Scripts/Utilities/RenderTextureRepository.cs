    ...
    internal static class RenderTextureRepository
    {
        ...
        public static RenderTexture Get(int id, Vector2 size, int rate, ref RenderTexture buffer, bool useStencil)
        {
            var preferSize = GetPreferSize(new Vector2Int(
                ...
                Mathf.Max(8, Mathf.RoundToInt(size.y))), rate, true);

            return Get(id, preferSize, ref buffer, useStencil);
        }

        public static RenderTexture Get(int id, Vector2Int size, ref RenderTexture buffer, bool useStencil)
        {
            Profiler.BeginSample("(SM4UI)[RTRepository] Get > ShouldToRelease");

            if (ShouldToRelease(buffer, size, useStencil))
            {
            ...
            Profiler.BeginSample("(SM4UI)[RTRepository] Get > Valid");
            ...
            Profiler.BeginSample("(SM4UI)[RTRepository] Get > Create");
        ...
        {
            Profiler.BeginSample("(SM4UI)[RTRepository] Release");
            s_Repository.Release(ref buffer);
            Profiler.EndSample();
        }

        public static Vector2Int GetPreferSize(Vector2Int size, int downSamplingRate, bool clampToScreen)
        {
        ...
        public static Vector2Int GetScreenSize(int rate)
        {
            return GetPreferSize(GetScreenSize(), rate, false);
        }
